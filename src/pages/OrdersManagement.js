...
export const OrdersManagement = () => {
  ...
  const printOrder = (order) => {
    // ESC/POS commands for 80MM thermal printer (302px width)
    const ESC = '\x1B';
    const GS = '\x1D';
    ...
    receipt += `#${order.order_number || order.id?.slice(0, 8)}\n`;
    receipt += GS + '!' + '\x00';
    receipt += ESC + 'E' + '\x00';
    ...
    // Heure de retrait/livraison demandée
    if (order.pickup_time || order.scheduled_time) {
      receipt += ESC + 'E' + '\x01';
      receipt += `⏰ HEURE: ${order.pickup_time || order.scheduled_time}\n`;
      receipt += ESC + 'E' + '\x00';
    }
    receipt += '================================\n\n';
    ...
    receipt += '👤 CLIENT\n';
    receipt += ESC + 'E' + '\x00';
    receipt += `Nom: ${order.customer_name || 'Client'}\n`;
    ...
    order.items?.forEach((item) => {
      ...
      if (item.selectedOptions) {
        // Si c'est une string
        if (typeof item.selectedOptions === 'string' && item.selectedOptions.trim()) {
          const options = item.selectedOptions.split(',').map(o => o.trim());
          options.forEach(opt => {
            if (opt) receipt += `   ▸ ${opt}\n`;
          });
        }
        // Si c'est un tableau
          ...
          item.selectedOptions.forEach(opt => {
            if (typeof opt === 'string') {
              ...
              receipt += `   ▸ ${opt.name}${opt.price > 0 ? ` (+${opt.price}€)` : ''}\n`;
            }
          });
        }
        // Si c'est un objet
      ...
      if (item.notes || item.comment) {
        receipt += `   💬 "${item.notes || item.comment}"\n`;
      }
      
      // Prix
      const itemPrice = item.total_price || item.totalPrice || (item.price * item.quantity);
      receipt += `   Prix: ${itemPrice?.toFixed(2) || '0.00'}€\n`;
      receipt += '\n';
    });
    ...
    // ========== COMMENTAIRE GÉNÉRAL ==========
    if (order.notes || order.order_notes || order.comment) {
      receipt += ESC + 'E' + '\x01';
      receipt += '💬 COMMENTAIRE CLIENT:\n';
      receipt += ESC + 'E' + '\x00';
      receipt += `"${order.notes || order.order_notes || order.comment}"\n`;
      receipt += '--------------------------------\n';
    }
    ...
    // Sous-total
    if (order.subtotal) {
      receipt += `Sous-total:           ${order.subtotal?.toFixed(2) || '0.00'}€\n`;
    }
    
    // ========== PROMOTIONS DÉTAILLÉES ==========
    if (order.promotions_applied && order.promotions_applied.length > 0) {
      receipt += '\n';
      receipt += ESC + 'E' + '\x01';
      receipt += '🏷️ PROMOTIONS APPLIQUÉES\n';
      receipt += ESC + 'E' + '\x00';
      
      order.promotions_applied.forEach((promo) => {
        const badge = promo.badge_text || promo.badge || '🏷️';
        const name = promo.promo_name || promo.name || 'Promo';
        const discount = promo.discount_amount || promo.discount || 0;
        receipt += `${badge} ${name}\n`;
        receipt += `   Réduction: -${discount?.toFixed(2)}€\n`;
        
        // Détails BOGO si présents
        if (promo.details && promo.details.free_items) {
          promo.details.free_items.forEach((item) => {
            receipt += `   🎁 ${item.quantity}x ${item.name} OFFERT\n`;
          });
        }
      });
      receipt += '\n';
    ...
    // ========== ARTICLES OFFERTS (RÉCOMPENSES) ==========
    const freeItems = order.items?.filter(item => item.price === 0 || item.fromReward === true);
    if (freeItems && freeItems.length > 0) {
      receipt += ESC + 'E' + '\x01';
      receipt += '🎁 RÉCOMPENSES UTILISÉES\n';
      receipt += ESC + 'E' + '\x00';
      
      freeItems.forEach((item) => {
        receipt += `   ✓ ${item.name} (OFFERT)\n`;
      });
      receipt += '\n';
    }
    
    // Réduction totale promos
    if (order.promotions_discount && order.promotions_discount > 0) {
      receipt += `Réductions promos:   -${order.promotions_discount?.toFixed(2)}€\n`;
    } else if (order.discount_amount && order.discount_amount > 0) {
      // Fallback pour anciennes commandes
      receipt += `Réductions:          -${order.discount_amount?.toFixed(2)}€\n`;
    }
    
    // Cashback utilisé
    if (order.cashback_used && order.cashback_used > 0) {
      receipt += `Fidélité utilisée:   -${order.cashback_used?.toFixed(2)}€\n`;
    }
    
    // Frais de livraison
    if (order.delivery_fee && order.delivery_fee > 0) {
      receipt += `Frais livraison:     +${order.delivery_fee?.toFixed(2)}€\n`;
    }
    ...
    receipt += GS + '!' + '\x11'; // Double size
    receipt += ESC + 'a' + '\x01'; // Center
    receipt += `TOTAL: ${order.total?.toFixed(2) || '0.00'}€\n`;
    receipt += GS + '!' + '\x00'; // Normal size
    receipt += ESC + 'E' + '\x00'; // Bold off
    ...
    const paymentMethodLabel = {
      ...
      'online': '🌐 En ligne',
      'on_site': '🏪 Sur place'
    }[order.payment_method] || order.payment_method || 'Non spécifié';
    
    receipt += `Moyen: ${paymentMethodLabel}\n`;
    
    // Statut paiement avec mise en évidence
    const isPaid = order.payment_status === 'paid';
    receipt += ESC + 'E' + '\x01';
    if (isPaid) {
      receipt += `Statut: ✅ PAYÉ\n`;
    } else {
      receipt += GS + '!' + '\x11'; // Double size pour attirer l'attention
      receipt += `⚠️ A ENCAISSER ⚠️\n`;
      receipt += GS + '!' + '\x00';
    }
    receipt += ESC + 'E' + '\x00';
    
    // Si paiement espèces avec rendu monnaie
    if (order.payment_method === 'cash' && order.amount_received) {
      receipt += `Reçu: ${order.amount_received?.toFixed(2)}€\n`;
      if (order.change_given) {
        receipt += `Rendu: ${order.change_given?.toFixed(2)}€\n`;
      }
    }
    ...
    // ========== CASHBACK GAGNÉ ==========
    if (order.cashback_earned && order.cashback_earned > 0) {
      receipt += '--------------------------------\n';
      receipt += ESC + 'a' + '\x01'; // Center
      receipt += `🎁 Fidélité gagnée: +${order.cashback_earned?.toFixed(2)}€\n`;
      receipt += ESC + 'a' + '\x00'; // Left
    }
    ...
    receipt += 'A bientot chez Family\'s!\n';
    receipt += '\n';
    receipt += `${new Date().toLocaleString('fr-FR')}\n`;
    receipt += '\n\n';
    ...
    const printWindow = window.open(url, '_blank', 'width=302,height=800');
    if (printWindow) {
      printWindow.onload = () => {
        printWindow.print();
        setTimeout(() => {
      ...
      a.href = url;
      a.download = `commande-${order.order_number || order.id?.slice(0, 8)}.txt`;
      a.click();
      URL.revokeObjectURL(url);
